    ...
    pub fn rebuild_flat_rows(&mut self, snapshot: &crate::arena::FileArenaSnapshot) {
        ...
        for (g_idx, group) in self.groups.iter().enumerate() {
            ...
            for (f_idx, &(node_idx, file_id)) in paired.iter().enumerate() {
                ...
                let parent_path = crate::arena::clean_unc_path(
                    &path
                        .parent()
                        .map_or_else(String::new, |s| s.to_string_lossy().into_owned()),
                )
...
pub fn is_excluded_path(path: &str, ignore_system: bool, ignore_hidden: bool) -> bool {
    if ignore_system {
        ...
        for pattern in &system_patterns {
            if crate::arena::contains_case_insensitive(path, pattern) {
                return true;
            }
        }
    }
...
mod tests {
    ...
    /// Build the same snapshot the scan coordinator would produce for a
    /// directory containing the given `(name, size)` files.
    fn dir_snapshot(dir: &std::path::Path, files: &[(&str, u64)]) -> FileArenaSnapshot {
        let mut pool = StringPool::new();
        let root_id = pool.get_or_insert(dir.to_string_lossy().as_bytes());
        let mut nodes = vec![FileNode::new(root_id, None, true, false, 0, 0)];
        for (i, (name, size)) in files.iter().enumerate() {
            let name_id = pool.get_or_insert(name.as_bytes());
            // Record the same timestamps a real scan would, so dedup's
            // phase-7 validation against the on-disk metadata passes.
            let (modified, created) = std::fs::metadata(dir.join(name)).map_or((0, 0), |meta| {
                (
                    meta.modified()
            ...
            let mut node = FileNode::new(name_id, Some(0), false, false, modified, created);
            node.size = *size;
            nodes.push(node);
            let idx = (i + 1) as u32;
            if i == 0 {
                nodes[0].first_child = idx;
            } else {
                nodes[i].next_sibling = idx;
            }
        }
