...
struct WalkCtx<'a> {
    ...
    traversal_stats: &'a crate::state::TraversalStats,
...
impl GuiApp {
            ...
            Some(ActiveModal::Delete) => {
                ...
                if idxs.is_empty() {
                    ...
                    let paths: Vec<String> = idxs
                        .iter()
                        .map(|&idx| {
                            crate::arena::clean_unc_path(&snapshot.get_full_path(idx)).into_owned()
                        })
                        .collect();
                    Some(ModalConfig {
    ...
    pub(crate) fn refresh_directory_subtrees(&mut self, dir_indices: &[u32]) {
        ...
        let traversal_stats = self.shared_state.scan_stats.clone();
...
mod tests {
    ...
    fn test_execute_softlinking() -> Result<(), crate::EdirstatError> {
        ...
        // Build the same snapshot the scan coordinator would produce for this directory
        let mut pool = StringPool::new();
        let root_id = pool.get_or_insert(temp_dir.to_string_lossy().as_bytes());
        let f1_id = pool.get_or_insert(b"file1.txt");
        let f2_id = pool.get_or_insert(b"file2.txt");
        // Record the same timestamps a real scan would, so dedup's phase-7
        // validation against the on-disk metadata passes.
        let ts_of = |path: &std::path::Path| {
        ...
        let (m1, c1) = ts_of(&file1_path);
        let (m2, c2) = ts_of(&file2_path);
        let mut nodes = vec![
            FileNode::new(root_id, None, true, false, 0, 0),
            FileNode::new(f1_id, Some(0), false, false, m1, c1),
            FileNode::new(f2_id, Some(0), false, false, m2, c2),
        ];
        nodes[0].first_child = 1;
        nodes[1].next_sibling = 2;
        nodes[1].size = content.len() as u64;
        nodes[2].size = content.len() as u64;
    ...
    fn test_gui_app_new_with_snapshot_file() -> Result<(), crate::EdirstatError> {
        ...
        crate::snapshot::save_snapshot(&nodes, &pool, &test_file, false)?; // Save as raw uncompressed

        let shared_state = Arc::new(SharedState::new());

        // Test loading snapshot file
        let mut app = GuiApp::new(shared_state.clone(), None, Some(test_file.clone()), false);
